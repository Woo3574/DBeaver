...
-- 1. EMP 테이블에서 사번, 사원명, 급여 조회  (단, 급여는 100단위까지의 값만 출력 처리하고 급여 기준 내림차순 정렬)CT EMPNO, ENAME, TRUNC(SAL, -2) FROM EMP
	ORDER BY SAL DESC;
...
SELECT EMPNO, ENAME, HIREDATE,
	ADD_MONTHS(HIREDATE, 480) AS "40주년"
	FROM EMP;
...
-- 실습 문제 : 노션  DATABASE -> 함수 파트 맨 아래에 있음
-- Q1.
SELECT EMPNO, RPAD(EMPNO,2) || '**' AS "MASKING_EMPNO",
	ENAME, RPAD(ENAME,1) || '****' AS "MASKING_ENAME"
	FROM EMP
